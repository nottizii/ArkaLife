...
import { Emoji, Message, ReactionUserManager } from "discord.js"

module.exports = {
    name: "prune",
    aliases: [],
    permissions: ['KICK_MEMBERS'],
    run: async(message: Message, args, client) => {
        ...
        m.awaitReactions((reaction, user) => (reaction.emoji.name === '❌' || reaction.emoji.name === '✅') && user.id === message.member.id, { time: 15000, errors: ['time'], max: 1 }).then(async collected => {
            if(collected?.first().emoji.name === '❌') return message.channel.send(new MessageEmbed().setDescription('❌ Comando cancelado'))
