...
module.exports = {
    ...
    run: async (oldCh, newCh) => {
        ...
        if (oldCh.name !== newCh.name)
            changes.push({ updated: 'Nombre', old: oldCh.name, newC: newCh.name });
        if (oldCh.permissionsLocked !== newCh.permissionsLocked)
            changes.push({ updated: 'Permisos sincronizados', old: oldCh.permissionsLocked ? 'Si' : 'No', newC: newCh.permissionsLocked ? 'Si' : 'No' });
        if (oldCh.rawPosition !== newCh.rawPosition)
            changes.push({ updated: 'Orden', old: oldCh.rawPosition, newC: newCh.rawPosition });
        if (oldCh.type !== newCh.type)
            changes.push({ updated: 'Tipo', old: oldCh.type, newC: newCh.type });
        if (oldCh.viewable !== newCh.viewable && newCh.deleted)
            changes.push({ updated: 'Mis permisos', old: oldCh.viewable ? 'Ver: Si' : 'Ver: No', newC: newCh.viewable ? 'Ver: Si' : 'Ver: No' });
